...
public class Plaque1 : Conversation {
	...
	protected override void GetContent (int key, out string content, out Dictionary<string, int> playerLines) {
		...
		case 1:
			...
			toContent = "Between twenty-five and thirty years ago, the Battle of Tanaab began in the orbit of the trade nexus world Tanaab.";
			AllowPlayerLines();
			playerLines = new Dictionary<string, int>() {
				{ "[Continue]", 2 }
			};
		...
		case 2:
			toContent = "The parties involved were the two factions of a planetary civil war, but, as has happened for millenia, more powerful groups began to funnel resources into this war to further their own interests.";
			AllowPlayerLines();
			playerLines = new Dictionary<string, int>() {
				{ "[Close]", 3 }
			};
